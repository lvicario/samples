 ...
 * Fim variables.
 */
var fimDir = 'http://banner.wo8cai.com/services/fim/7/'
,	fimName = 'fim'
,	gtsSwf = 'http://banner.wo8cai.com/gtsclient.html'
,	gtsMode = null
,	pokerMode = null
,	gtsMiniMode = null
,	userKey = User.key
...
function fimReady() {
	...
	fim.initClient("gts", gtsSwf + '?language=' + language);
	
	if (userKey != '') {
		fim.initUser(userKey); 
	}
	
	if (gameHash && gameCode.length != gameHash.length && gameHash.indexOf('overlay') == -1 && User.authenticated) {
		launchFimGTS(gameCode);
	}
}
 ...
 * Fim callback.
 */
function launchFimGTS(module, offline) {
	var fim = getFim(fimName)
	,	offline = typeof offline !== 'undefined' ? offline : false;
	
	if (userKey != "" && offline == false) {
		fim.launchClient("gts", module);
	} else {
		fim.launchClient("gts", module, 'offline');
	}
}
